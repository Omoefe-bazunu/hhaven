export const mockHymns = [
  {
    id: '1',
    translations: {
      en: {
        title: 'Amazing Grace',
        content:
          "Amazing grace how sweet the sound\nThat saved a wretch like me\nI once was lost, but now I'm found\nWas blind but now I see",
        audioUrl: 'https://example.com/amazing-grace-en.mp3',
      },
      yo: {
        title: 'Oore-ofe Ti O Dun',
        content:
          'Oore-ofe ti o dun to\nTi o gba eniyan bi mi\nMo ti sonu, sugbon bayi mo ri\nMo fooju, bayi mo ri',
        audioUrl: 'https://example.com/amazing-grace-yo.mp3',
      },
      fr: {
        title: 'Grâce Étonnante',
        content:
          "Grâce étonnante, que ce son est doux\nQui a sauvé un misérable comme moi\nJ'étais perdu, mais maintenant je suis trouvé\nJ'étais aveugle mais maintenant je vois",
        audioUrl: 'https://example.com/amazing-grace-fr.mp3',
      },
    },
    createdAt: '2025-01-15T10:00:00Z',
  },
  {
    id: '2',
    translations: {
      en: {
        title: 'How Great Thou Art',
        content:
          'O Lord my God, when I in awesome wonder\nConsider all the worlds thy hands have made\nI see the stars, I hear the rolling thunder\nThy power throughout the universe displayed',
        audioUrl: 'https://example.com/how-great-thou-art-en.mp3',
      },
      yo: {
        title: 'Bi O Ti Tobi To',
        content:
          'Oluwa Olorun mi, nigbati mo ba wo\nGbogbo aiye ti owo re da\nMo ri awon irawọ, mo gbo ariwo ara\nAgbara re ni gbogbo aiye',
        audioUrl: 'https://example.com/how-great-thou-art-yo.mp3',
      },
    },
    createdAt: '2025-01-14T10:00:00Z',
  },
];

export const mockSermons = [
  {
    id: '1',
    translations: {
      en: {
        title: 'Walking in Faith',
        content: `What you are about to learn in this book might come as a shock to you as it might not be exactly what you were expecting based on your present knowledge of the subject. Be rest assured that it is in no way a mere opinion but is in fact the uncommon and unpopular truth which many like yourself might have overlooked when going through the pages of the holy bible, or have never really heard or paid careful attention to. If we asked you what you think the gift of speaking in tongues is and how important it is to Christians, your answer might be one of the following;
Option 1: The gift of tongues is a heavenly language that is given to a Christian when he/she receives the holy spirit from God after conversion. It is used to communicate with God in a special way that cannot be interrupted by Satan or his agents.
Option 2: The gift of tongues is a spiritual language that Christians can use to cast out Satan and his demons and to elevate their prayers in a way that provides desired and fast results.
If your answer was any one of the above we humbly encourage you the more to keep reading this book as you will be discovering some really interesting truth.
You will agree that as Christians we are commanded and bound to worship God Almighty and our Savior Jesus Christ based on the truth as we find it in the holy bible and not based on our personal opinion or the opinions of any Christian group. See 2Peter 1:20; Matthew 15:7-9.
More so, a Christian who sincerely desires to worship God in the way that pleases God will have an open mind to learn the truth of God’s word and keep to it even when it requires such person to put aside some long-standing ideas and practices which he has now found out to not be in harmony with the truth.
On this subject of SPEAKING IN TONGUES, to give a proper answer to what it really is, we rely solely on the record of the holy bible where the origin of the gift can be found. At this point we encourage you to get a copy of the holy bible and turn to the book of ACTS 2: 1-14. This is the passage where we find the origin or beginning of the gift of tongues and where we can learn what exactly it is and what it was meant for.
`,
        audioUrl: 'https://example.com/walking-in-faith-en.mp3',
      },
      yo: {
        title: 'Ririn Ninu Igbagbo',
        content:
          'Loni a yoo wo kini o tumọ si lati rin ninu igbagbo, gbara Olorun gbekele paanù koda nitori a ko le ri ọna...',
        audioUrl: 'https://example.com/walking-in-faith-yo.mp3',
      },
    },
    date: '2025-01-12',
    duration: '45:30',
  },
  {
    id: '2',
    translations: {
      en: {
        title: 'The Power of Prayer',
        content:
          'Prayer is our direct line to God. Through prayer, we find strength, guidance, and peace...',
        audioUrl: 'https://example.com/power-of-prayer-en.mp3',
      },
      yo: {
        title: 'Agbara Adura',
        content:
          'Adura ni ọna wa taara si Olorun. Nipa adura, a wa agbara, itọsọna, ati alaafia...',
        audioUrl: 'https://example.com/power-of-prayer-yo.mp3',
      },
    },
    date: '2025-01-05',
    duration: '38:15',
  },
];

export const mockSongs = [
  {
    id: '1',
    title: 'Joyful Instrumental',
    category: 'native',
    style: 'Contemporary Gospel',
    url: 'https://example.com/joyful-instrumental.mp3',
    duration: '4:23',
  },
  {
    id: '2',
    title: 'Hallelujah Chorus',
    category: 'acappella',
    style: 'Traditional Gospel',
    url: 'https://example.com/hallelujah-chorus.mp3',
    duration: '3:45',
  },
  {
    id: '3',
    title: 'French Worship',
    category: 'english',
    style: 'French Gospel',
    url: 'https://example.com/french-worship.mp3',
    duration: '5:12',
  },
];

export const mockVideos = [
  {
    id: '1',
    title: 'David and Goliath',
    duration: '8:30',
    languageCategory: 'English',
    url: 'https://example.com/david-goliath.mp4',
    thumbnailUrl:
      'https://images.pexels.com/photos/8879724/pexels-photo-8879724.jpeg?auto=compress&cs=tinysrgb&w=800',
  },
  {
    id: '2',
    title: "Noah's Ark",
    duration: '12:15',
    languageCategory: 'Multi-language',
    url: 'https://example.com/noahs-ark.mp4',
    thumbnailUrl:
      'https://images.pexels.com/photos/15031382/pexels-photo-15031382.jpeg?auto=compress&cs=tinysrgb&w=800',
  },
  {
    id: '3',
    title: 'The Good Samaritan',
    duration: '6:45',
    languageCategory: 'English',
    url: 'https://example.com/good-samaritan.mp4',
    thumbnailUrl:
      'https://images.pexels.com/photos/8879620/pexels-photo-8879620.jpeg?auto=compress&cs=tinysrgb&w=800',
  },
];

export const mockAboutInfo = {
  version: '1.0.0',
  content:
    "Haven is a multilingual church app designed to bring the gospel to people of all languages and backgrounds. Our mission is to spread God's love through hymns, sermons, music, and animated Bible stories.",
  contactEmail: 'info@higher.com.ng',
  churchMission:
    'To glorify God and make disciples of all nations through multilingual worship and biblical teaching.',
};
